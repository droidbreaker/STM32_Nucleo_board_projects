...
int32_t add_numbers(int32_t x , int32_t y )
{
    ...
    __asm volatile ("MOV %0,R0": "=r"(res) ::);     // moving the x and y into the r0 and r1 register
    return res;
...
/*SVC number signifies the type of operation , decode it */
    ...
    {
    case 36:
        printf("svcno: 36 (addition): pBaseOdStackFrame[0] = %d \n",pBaseOfStackFrame[0]);  // r0 data
        printf("svcno: 36 (addition): pBaseOdStackFrame[1] = %d \n",pBaseOfStackFrame[1]);  // r1 data
        result = pBaseOfStackFrame[0] + pBaseOfStackFrame[1];
        break;
    case 37:
    	printf("svcno: 37 (subtraction): pBaseOdStackFrame[0] = %d \n",pBaseOfStackFrame[0]);  // r0 data
    	printf("svcno: 37 (subtraction): pBaseOdStackFrame[1] = %d \n",pBaseOfStackFrame[1]);  // r1 data
    	result = pBaseOfStackFrame[0] - pBaseOfStackFrame[1];
        break;
    case 38:
    	printf("svcno: 38 (multiplication): pBaseOdStackFrame[0] = %d \n",pBaseOfStackFrame[0]);  // r0 data
    	printf("svcno: 38 (multiplication): pBaseOdStackFrame[1] = %d \n",pBaseOfStackFrame[1]);  // r1 data
    	result = pBaseOfStackFrame[0] * pBaseOfStackFrame[1];
        break;
    case 39:
    	printf("svcno: 39 (division): pBaseOdStackFrame[0] = %d \n",pBaseOfStackFrame[0]);  // r0 data
    	printf("svcno: 39 (division): pBaseOdStackFrame[1] = %d \n",pBaseOfStackFrame[1]);  // r1 data
        result = pBaseOfStackFrame[0] / pBaseOfStackFrame[1];
    	break;
    default:
